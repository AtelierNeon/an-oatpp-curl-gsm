...
#include "ProxyRequestExecutor.hpp"
...
ProxyRequestExecutor::ProxyRequestExecutor(const oatpp::String& baseUrl, const oatpp::String& proxyUrl, bool verbose)
...
std::shared_ptr<ProxyRequestExecutor::ConnectionHandle> ProxyRequestExecutor::getConnection() {
  return std::make_shared<StubConnectionHandle>();
}


oatpp::async::CoroutineStarterForResult<const std::shared_ptr<ProxyRequestExecutor::ConnectionHandle>&> ProxyRequestExecutor::getConnectionAsync() {

  class ConnectionCoroutine : public oatpp::async::CoroutineWithResult<ConnectionCoroutine, const std::shared_ptr<ProxyRequestExecutor::ConnectionHandle>& > {
  public:
...
std::shared_ptr<ProxyRequestExecutor::Response> ProxyRequestExecutor::executeOnce(const String& method,
                                                                                  const String& path,
                                                                                  ...
                                                                                  const std::shared_ptr<ConnectionHandle>& connectionHandle)
{
  ...
  oatpp::String emptyProxy = "";
  if (emptyProxy.equalsCI_ASCII(m_proxyUrl)) {
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_SSL_OPTIONS, CURLSSLOPT_NATIVE_CA);
  } else {
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_PROXY, m_proxyUrl->c_str());
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_PROXY_SSL_OPTIONS, CURLSSLOPT_NATIVE_CA | CURLSSLOPT_NO_REVOKE);
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_PROXY_SSL_VERIFYHOST, 0);
    /*
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_PROXY_SSL_VERIFYPEER, false);
    */
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_SSL_OPTIONS, CURLSSLOPT_NATIVE_CA | CURLSSLOPT_NO_REVOKE);
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_SSL_VERIFYHOST, 0);
    /*
    curl_easy_setopt(curl->getEasyHandle(), CURLOPT_SSL_VERIFYPEER, false);
    */
  }
  ...
  if(bodyHeaders.getSize() > 0) {
    ...
    auto knownBodySize = body->getKnownSize();
    if (knownBodySize >= 0) {
      curl_easy_setopt(curl->getEasyHandle(), CURLOPT_INFILESIZE_LARGE, knownBodySize);
    }
...
oatpp::async::CoroutineStarterForResult<const std::shared_ptr<ProxyRequestExecutor::Response>&>
ProxyRequestExecutor::executeOnceAsync(const String& method,
                                       const String& path,
                                       ...
                                       const std::shared_ptr<ConnectionHandle>& connectionHandle)
{
  ...
  public:
      ...
      , m_verbose(verbose)
    {
      ...
      oatpp::String emptyProxy = "";
      if (emptyProxy.equalsCI_ASCII(proxy)) {
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_SSL_OPTIONS, CURLSSLOPT_NATIVE_CA);
      } else {
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_PROXY, proxy->c_str());
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_PROXY_SSL_OPTIONS, CURLSSLOPT_NATIVE_CA | CURLSSLOPT_NO_REVOKE);
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_PROXY_SSL_VERIFYHOST, 0);
        /*
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_PROXY_SSL_VERIFYPEER, false);
        */
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_SSL_OPTIONS, CURLSSLOPT_NATIVE_CA | CURLSSLOPT_NO_REVOKE);
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_SSL_VERIFYHOST, 0);
        /*
        curl_easy_setopt(m_curl->getEasyHandle(), CURLOPT_SSL_VERIFYPEER, false);
        */
      }
  ...
  return ExecutorCoroutine::startForResult(m_baseUrl + path, m_proxyUrl, method, headers, body, m_bodyDecoder, m_verbose);
